...
def plot_gc_ratio_for_a_read(read_GC_avg, pct, figname=False):
    ''' Plot the GC ratio trought a read '''
...
def plot_GC_content(gc_con, title='', figname=False):
    plt.figure(figsize=(6,4))
    plt.plot(gc_con, 'o', c='b')
    plt.title(title, fontsize=FS+3)
    plt.xlabel('Reads', fontsize=FS)
    plt.ylabel('GC content [%]', fontsize=FS)
    plt.grid()
    if figname:
        plt.savefig(figname, bbox_inches='tight')
