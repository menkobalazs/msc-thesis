...
def find_the_loneliest_base_in_seq(seq, base='T', verbose=0):
    ...
    if verbose:
        print(f"The position of the loneliest base '{base}': {pos_of_loneliest_base}")
        print(f"Distances between the neighborhood: {distances[idx_of_loneliest_base-2:idx_of_loneliest_base+2]}")
...
def create_df_to_violin_plot(dataset, set_name, num_of_left_neighbors, num_of_right_neighbors, search_position):
    ...
    for entry in tqdm(dataset, total=len(dataset), desc='Processing data'):
        idx = find_the_loneliest_base_in_seq(entry['ref_seq'])
